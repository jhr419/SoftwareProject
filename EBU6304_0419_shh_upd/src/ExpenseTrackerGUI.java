...
public class ExpenseTrackerGUI {
    ...
    private void initialize() {
        ...
        mainPanel.setBackground(new Color(240, 240, 240));  // 淡灰色背景

        // 标题部分
        JPanel titlePanel = new JPanel();
        titlePanel.setBackground(new Color(33, 150, 243));  // 蓝色背景
        JLabel titleLabel = new JLabel("财务管理系统", SwingConstants.CENTER);
        ...
        // Transaction Management 部分
        addSection(mainPanel, "交易管理", new String[]{"添加交易", "显示所有交易", "显示分类支出", "加载CSV数据"}, new ActionListener[]{
                e -> addExpense(),
                e -> expenseManager.displayExpenses(),
                e -> expenseManager.displayCategoryExpenses(),
                e -> showExpenseTable()
        });

        // Budget Management 部分
        addSection(mainPanel, "预算管理", new String[]{"设置预算", "移除预算", "显示预算报告", "显示预算进度"}, new ActionListener[]{
                e -> setBudget(),
                e -> removeBudget(),
                e -> showBudgetReport(),
                e -> showBudgetProgress()
        });

        // Data Visualization 部分
        addSection(mainPanel, "数据可视化", new String[]{"显示时间线图", "显示分类饼图"}, new ActionListener[]{
                e -> showTimeLineChart(),
                e -> showCategoryPieChart()
        });

        // Savings 部分
        addSection(mainPanel, "储蓄管理", new String[]{"设置储蓄", "显示储蓄报告"}, new ActionListener[]{
                e -> setSavings(),
                e -> showSavingsReport()
        });

        // Reports 部分
        addSection(mainPanel, "报告", new String[]{"显示分类报告"}, new ActionListener[]{
                e -> showClassificationReport()
        });
